...
class DenseboxTarget(TargetBase):
    ...
    def __call__(self, sampled_data: Dict) -> Dict:
        ...
        for i in range(nmf):
            ...
            mask_ms.append(data_m['mask_{}'.format(i)])

        data_q = sampled_data["data2"]
        im_q, bbox_q,mask_q = data_q["image"], data_q["anno"],data_q["mask"]

        is_negative_pair = sampled_data["is_negative_pair"]

        # input tensor
        im_m = np.stack(im_ms, axis=0)
        #print("im_m : ",im_m.shape)
        im_m = im_m.transpose(0, 3, 1, 2)  # T, C, H, W
        #print("im_m : ",im_m.shape)

        mask_m=np.stack(mask_ms, axis=0)
        #print("mask_m : ",mask_m.shape)
        #mask_m = mask_m.transpose(0, 3, 1, 2) # T, C, H, W
        # print("mask_m : ",mask_m.shape)
        # input()
        im_q = im_q.transpose(2, 0, 1)
        mask_q=np.stack([mask_q], axis=0)
        #mask_q = mask_q.transpose(2, 0, 1)
