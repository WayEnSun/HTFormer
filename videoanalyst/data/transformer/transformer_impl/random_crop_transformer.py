...
class RandomCropTransformer(TransformerBase):
    ...
    def __call__(self, sampled_data: Dict) -> Dict:
        ...
        for i in range(nmf):
            ...
            im_m, bbox_m, mask_m,_ = crop(im_memory, bbox_memory, search_area_factor, q_size,
            ...
            sampled_data["data1"]['mask_{}'.format(i)] = mask_m
        im_query, bbox_query = data2["image"], data2["anno"]
        im_q, bbox_q, mask_q,_ = crop(im_query, bbox_query, search_area_factor, q_size,
                               config=self._hyper_params, rng=self._state["rng"])
        sampled_data["data2"] = dict(image=im_q, anno=bbox_q,mask=mask_q)
