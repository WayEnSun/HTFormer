...
def _sample_target_adaptive(im, target_bb: np.ndarray, search_area_factor, output_sz,
    ...
    if warp_affine:
        ...
        # a_m_1=att_mask.reshape((H,W,-1)).astype(np.uint8)*255
        # cv2.imshow("att_mask 1",a_m_1)
        # cv2.imshow("im_crop_padded",im_crop_padded)
        im_out = cv2.resize(im_crop_padded, tuple(output_sz.tolist()))
        att_mask = cv2.resize(att_mask,tuple(output_sz.tolist())).astype(np.bool_)
        # att_mask_shape=output_sz.tolist()
        # att_mask_shape.append(1)
        # att_mask=att_mask.reshape(tuple(att_mask_shape))
        #cv2.imshow("mask im_out",im_out)
        #a_m_2=att_mask.reshape((289,289,1)).astype(np.uint8)*255
    ...
    return im_out, bbox_out, att_mask,scale_factor
...
def crop(im, target_bb: np.ndarray, search_area_factor, output_sz,
    ...
    im_out, bbox_out, mask_out,scale_factor = _sample_target_adaptive(im, target_bb, search_area_factor, output_sz,
                                                             warp_affine=False,
                                                             avg_chans=avg_chans,
                                                             translation=(dx, dy))
    ...
    return im_out, bbox_out, mask_out,scale_factor
