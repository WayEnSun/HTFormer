...
class AiAModule(Module):
    ...
    def aia_attention_forward(
        ...
        if key_padding_mask is not None:
            ...
            # print("bsz, 1, 1, src_len : ",bsz, 1, 1, src_len)
            # input("aia")
            key_padding_mask = key_padding_mask.view(bsz, 1, 1, src_len). \
                expand(-1, num_heads, -1, -1).reshape(bsz * num_heads, 1, src_len)
            if attn_mask is None:
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, src,mem,
        ...
        for stack, layer in enumerate(self.layers):
            output = layer(output, mem,src_key_padding_mask=src_key_padding_mask, pos=pos, inr=inr)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 ...
                 use_AiA=True, match_dim=64, feat_size=400):
        super().__init__()
        self.self_cross_attn = AiAModule(d_model, nhead, dropout=dropout,
                                   use_AiA=use_AiA, match_dim=match_dim, feat_size=feat_size)
        # Implementation of Feedforward model
        self.linear1 = nn.Linear(d_model, dim_feedforward)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, src,
        ...
        src2 = self.self_cross_attn(query=src, key=mem, value=mem, pos_emb=inr, key_padding_mask=src_key_padding_mask)[0]
        # Add and norm
        src = src + self.dropout1(src2)
        src = self.norm1(src)
        # FFN
...
if __name__== "__main__" :
    ...
    self_attn=transformer.encoder.layers[0].self_attn

    feat=torch.randn(400, 1, 256).requires_grad_(True)
    mask=torch.randn(1, 400).requires_grad_(True)
    inr=torch.randn(400, 1, 64).requires_grad_(True)
    pos=torch.randn(400, 1, 256).requires_grad_(True)
    ...
    inr=None
    net_plot = make_dot(
        self_attn(query=q, key=k, value=feat, pos_emb=inr, key_padding_mask=mask),
        params = dict(list(self_attn.named_parameters()) 
        + [('q', q)]+ [('k', k)]+ [('v', feat)]))#+ [('inr', inr)]))#+ [('mask', mask)]))
    net_plot.view()

    # feat = torch.rand(1, 256, 20, 20).requires_grad_(True)
    # feat = feat.flatten(2).permute(2, 0, 1)

    # mask = torch.rand(1, 20, 20)
    # mask = mask.flatten(1)

    # inr  = torch.rand(1, 64, 20, 20)
    # inr  = inr.flatten(2).permute(2, 0, 1)

    # pos  = torch.rand(1, 256, 20, 20)
    # pos = pos.flatten(2).permute(2, 0, 1)

    # feat=torch.cat([feat], dim=0)
    # mask=torch.cat([mask], dim=1)
    # pos=torch.cat([pos],dim=0)
    # inr=torch.cat([inr],dim=0)
    # print("feat : ",feat.shape)
    # print("mask : ",mask.shape)
    # print("inr : ",inr.shape)
    # print("pos : ",pos.shape)
    
    # feat :  torch.Size([400, 1, 256])
    # mask :  torch.Size([1, 400])
    # inr :  torch.Size([400, 1, 64])
    # pos :  torch.Size([400, 1, 256])
    # q=(q+pos).requires_grad_(True)
    # k=(k+pos).requires_grad_(True)
    # self_attn = AiAModule(embed_dim=256, 
    #                       num_heads=4, 
    #                       dropout=0.1,
    #                       use_AiA=True, 
    #                       match_dim=64, 
