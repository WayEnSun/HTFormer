...
from videoanalyst.model.aia_transformer.transformer_impl.transformer import (TransformerDecoderLayer,
                                                                            TransformerDecoder)    
...
class STMHead(ModuleBase):
    ...
    def forward(self, fm, fq_list, q_size=0):

        fq=fq_list["fq"]
        fq_mask=fq_list["fq_mask"]
        fq_pos=fq_list["fq_pos"]
        fq_inr=fq_list["fq_inr"]

        mem,fq = self.memory_read(fm, fq)

        b,c,w,h=fq.size()
        fq=(fq).view(b,c,-1).permute(2, 0, 1).contiguous()
        mem=(mem).view(b,c,-1).permute(2, 0, 1).contiguous()

        # print("fq : ",fq.shape)
        # print("mem : ",mem.shape)
        # print("fq_mask : ",fq_mask.shape)
        # print("fq_pos : ",fq_pos.shape)
        # print("fq_inr : ",fq_inr.shape)
        y=self.decoder(fq, mem,src_key_padding_mask=fq_mask, pos=fq_pos, inr=fq_inr)
        y=y.permute(1,2,0).view(b,c,w,h)
    ...
    def _make_net(self):
        ...
        self.cls_ctr = ConvModule(self.in_channels, mdim)
        self.reg = ConvModule(self.in_channels, mdim)
