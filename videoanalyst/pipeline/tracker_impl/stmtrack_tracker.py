...
class STMTrackTracker(PipelineBase):
    ...
    def memorize(self, im: np.array, target_pos, target_sz, avg_chans):
        ...
        im_m_crop, real_scale,im_m_mask = get_crop_single(im, target_pos, scale_m, m_size, avg_chans)
        #m_out, bbox_out, mask_out,scale_factor = crop(im, target_bb, scale_m, m_size,avg_chans)
        phase = self._hyper_params['phase_memorize']
        with torch.no_grad():
            data = imarray_to_tensor(im_m_crop).to(self.device)
            mask=imarray_to_tensor(im_m_mask).to(self.device)
            ...
            fm = self._model(data,mask, fg_bg_label_map, phase=phase)
        return fm
    ...
    def select_representatives(self, cur_frame_idx):
        ...
        #print("representatives : ",representatives.shape)
    ...
    def track(self,
        ...
        im_q_crop, scale_q, im_q_mask= get_crop_single(im_q, target_pos, self._state['target_scale'], q_size, avg_chans)
        #im_q_mask=?
        self._state["scale_q"] = deepcopy(scale_q)
        with torch.no_grad():
            score, box, cls, ctr, extra = self._model(
                imarray_to_tensor(im_q_crop).to(self.device),
                imarray_to_tensor(im_q_mask).to(self.device),
                features,
                phase=phase_track)
