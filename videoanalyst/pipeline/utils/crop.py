...
def get_crop_numpy(im: np.ndarray, pos: np.ndarray, sample_sz: np.ndarray, output_sz: np.ndarray = None,
    ...
    # print("M_13 M_23 : ",M_13,M_23)
    # print("mat2x3 : ",mat2x3)
    # cv2.imshow("im2",im2)
    # print("im2 : ",im2.shape)

    # output_sz[0]=output_sz[0]*2
    # output_sz[1]=output_sz[1]*2
    x1=max(-int(M_13/M_11),0)
    y1=max(-int(M_23/M_22),0)
    x2=min(int(x1+im2.shape[1]/M_11),output_sz[0])
    y2=min(int(y1+im2.shape[0]/M_22),output_sz[1])
    att_mask = np.ones((output_sz[0], output_sz[1],1))

    # print("x1:x2,y1:y2 : ",x1,x2,y1,y2)
    att_mask[y1:y2,x1:x2,:] = 0

    # print("att_mask : ",att_mask.shape)
    # a_m_2=att_mask.reshape((output_sz[0], output_sz[1],1)).astype(np.uint8)*255
    #cv2.imshow("mask a_m_2",a_m_2)
    ...
    # print("scale : ",scale)
    # cv2.imshow("im_patch 1",im_patch)
    # cv2.imshow("im_patch_2",im_patch_2)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()

    return im_patch, patch_coord, scale,att_mask.astype(np.bool_)
...
def get_crop_single(im: np.ndarray, target_pos: np.ndarray, target_scale: float, output_sz: int, avg_chans: tuple):
    ...
    im_patch, _, scale_x,im_mask = get_crop_numpy(im, pos, sample_sz, output_sz, avg_chans=avg_chans)
    return im_patch, scale_x[0].item(),im_mask
